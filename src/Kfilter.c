 ...
 int *Debug)
{
  ...
  int k, l, m, n;					// reserved for matrix calculation loops in macro's, do not use with these macro's!
	int debug = *Debug, false = *Debug, missing = 0;
  double detF;
	...
	if(debug){
		...
		Rprintf("nt = %d\nne = %d\nnm = %d\nny = %d\nnx = %d\nauc = %g, %g, %g, %g\n",*NT,ne,nm,ny,nx,auc[0],auc[1],auc[2],auc[3]);
		Rprintf("H = %g, %g\tW = %g, %g\tG = %g, %g\tB = %g\tR = %g\n",H[0],H[1],W[0],W[1],G[0],G[1],B[0],R[0]);
		Rprintf_matrix("H[%d,%d] = %g\t", &H[0], ne, ne);
		Rprintf("-----------------------------------------------------------------------------------------------------------\n");
    ...
    for(t=0; t<nt; t++){
        ...
        for(i=0; i<nm; i++)
        {
			...
			if(debug && i==0 && j==0) {
				Rprintf("\nt = %d\n", t);
				Rprintf_matrix("\tc[%d,%d] = %g", &_c(__, j), ne, 1); Rprintf("\n");
				Rprintf_matrix("\tH[%d,%d] = %g", &_H(__, __, j), ne, ne); Rprintf("\n");
				Rprintf_matrix("\tauc[%d,%d] = %g", &__auc(__, i, t), ne, 1);  Rprintf("\n");
				Rprintf_matrix("\tap[%d,%d] = %g", &__ap(__, i, j, t), ne, 1);  Rprintf("\n");
			}
			/*** ----------------- end debug ------------------- ***/
			...
			// HPucH = H * Puc * H'
            a_matmulABAt(&_H(0,0,j), ne, ne, &__Puc(__, __, i, t), HPucH );                                                             // H[][][j] = H[0+ne*0+ne*ne*j], Puc[][][i][t] = Puc[0+ne*0+ne*ne*i+ne*ne*nm*t]
			//
            a_matmulABAt(&_G(__, __, j), ne, ne, &_K(__, __, j), GKG);                                                                // G[][][j] = G[0 + ne*0 + ne*ne*j], K[][][j] = K[0 + ne*0 + ne*ne*j]
            // Pp = H * Puc * H'  + G * K * G'
            a_mataddAB(HPucH, ne, ne, GKG, &__Pp(__, __, i, j, t));                       // Pp[][][i][j][t] = Pp[0+ne*0+ne*ne*i+ne*ne*nm*j+ne*ne*nm*nm*t]
			...
			if(debug && false){
				Rprintf("\n");
				Rprintf_matrix("\tPuc[%d,%d] = %12.8g\n", &__Puc(__, __, j, t),    ne, ne); Rprintf("\n");
				Rprintf_matrix("\tG[%d,%d] = %g\n",       &   _G(__, __, j),       ne, ne); Rprintf("\n");
				Rprintf_matrix("\tK[%d,%d] = %12.8g\n",   &   _K(__, __, j),       ne, ne); Rprintf("\n");
				Rprintf_matrix("\tPp[%d,%d] = %12.8g\n",  & __Pp(__, __, i, j, t), ne, ne);
			}
			/*** ----------------- end debug ------------------- ***/
			...
			if(!missing) {
				...
				if(debug){
					Rprintf("\n");
					Rprintf_matrix("\ty[%d,%d] = %12.8g\n",  & __y(__,  t),       ny,  1);
					Rprintf_matrix("\tW[%d,%d] = %12.8g\n",  &  _W(__, __, j),    ny, ne);
					Rprintf_matrix("\tap[%d,%d] = %12.8g\n", &__ap(__,  i, j, t), ne,  1);
					Rprintf_matrix("\tv1[%d,%d] = %12.8g\n", v1, ne, 1);
				}
				/*** ----------------- end debug ------------------- ***/
				...
				/*** ----------------- debug code ------------------- ***/
				if(debug && false){
					Rprintf_matrix("\tB[%d,%d] = %12.8g\n", & _B(__, __, j), ny, nx);
					Rprintf_matrix("\tv[%d,%d] = %12.8g\n", &__v(__,  i, j, t), ny, 1);
				}
				/*** ----------------- end debug ------------------- ***/
			}
			...
			if(debug && false){
				Rprintf("\n");
				Rprintf_matrix("\tR[%d,%d] = %12.8g", & _R(__, __, j),       ny, ny); Rprintf("\n");
				Rprintf_matrix("\tF[%d,%d] = %12.8g", &__F(__, __, i, j, t), ny, ny); Rprintf("\n");
			}
			/*** ----------------- end debug ------------------- ***/
			...
			if(missing) {
				...
				// updated state with expected value under Markov-condition
				for(m=0;m<ne;m++){
			    ...
			    vec2mat(&__F(__, __, i, j, t), ny, ny, _F); // double* -> double**,  F[][][i][j][t] = F[0+ny*0+ny*ny*i+ne*ne*nm*j+ne*ne*nm*nm*t]
				...
				if(debug && false){
					Rprintf("\n");
					Rprintf_matrix("\tFinv[%d,%d] = %12.8g", Finv, ny, ny); Rprintf("\n");
				}
				/*** ----------------- end debug ------------------- ***/
			...
			if(debug){
				...
				Rprintf("\n");
				Rprintf_matrix("\tPu[%d,%d] = %12.8g", &__Pu(__, __, i, j, t), ne, ne);
			}
			/*** ----------------- end debug ------------------- ***/
			...
			if(!missing){
				...
				/*** ----------------- debug code ------------------- ***/
				if(debug && false)
					Rprintf("\tv = %g, Finv = %g, vtFinvv[0] = %g\n", __v(0, i, j, t), Finv[0], vtFinvv[0]);
				/*** ----------------- end debug ------------------- ***/
				...
				/*** ----------------- debug code ------------------- ***/
				if(debug)
					Rprintf("\tp = %g, pr = %g, tpr = %g\n\t(2*pi)^(-ny/2) = %g, vtFinvv[0] = %g, detF = %g\n\tjd = %g\n", _p(j, i), __pr(i, t), __tpr(j,i,t), pow(M_2PI,-(double)ny/2.0), vtFinvv[0], detF, __jd(j, i, t));
				/*** ----------------- end debug ------------------- ***/
		...
		if(debug){
			Rprintf("\n");
			Rprintf_matrix("\tutpr[%d,%d] = %12.8g", &__utpr(__, __, t), nm, nm);
		}
		/*** ----------------- end debug ------------------- ***/
        ...
        for(j=0; j<nm; j++){ // loop over regimes
			...
			if(debug){
				Rprintf("\n");
				Rprintf_matrix("\tpr[%d,%d] = %12.8g", &__pr(__, t+1), nm, 1);
			}
			/*** ----------------- end debug ------------------- ***/
			...
			if(debug){
				Rprintf("\n");
				Rprintf_matrix("\tas[%d,%d] = %12.8g", as, ne, 1);
				Rprintf_matrix("\tau[%d,%d] = %12.8g", &__au(__, __, j, t), ne, nm);
			}
			/*** ----------------- end debug ------------------- ***/
			...
			for(i=0; i<nm; i++){
				...
				/*** ----------------- debug code ------------------- ***/
				if(debug){
					Rprintf("\n\tauc(j,t+1) - au(i,j,t) = %12.8g", __auc(0, j, t+1)-__au(0, i, j, t));
					Rprintf("\n\tutpr(i,j,t) / pr(j,t+1) = %12.8g", __utpr(i, j, t) / __pr(j,t+1));
				}
				/*** ----------------- end debug ------------------- ***/
			}
			...
			if(debug){
				Rprintf("\n");
				Rprintf("\tj= %d\n", j);
				Rprintf_matrix("\tPu[%d,%d] = %12.8g", &__Pu(__,__, 0, j, t), ne, ne);
				Rprintf_matrix("\tPs[%d,%d] = %12.8g", Ps, ne, ne);
			}
			/*** ----------------- end debug ------------------- ***/
